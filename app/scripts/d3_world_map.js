...
const title = titleContainer.append('h1')
...
const button = buttonContainer.append('a')
  .text('Reset points')
  .style('margin', '0 auto')
  .attr('class', 'button')
  .on('click', () => {
...
const emigrationTip = d3.tip()
  .attr('class', 'd3-tip')
  .offset([-12, 0])
  .html((d, i) => `Emigration to ${d[0].split("_").join(" ")} : 
...
function displayEmigration(dest, origin, max, centroids) {
  ...
  let strokes = []
  dest.forEach(e => {
    strokes.push([drawPath(d3.path(), origin, getCentroid(e[0].split("_").join(" "), centroids)), e[0]])
  })

  svg.selectAll('.emigration-line').data(dest).enter()
    .append('path')
  ...
  svg.selectAll('.emigration-data').data(dest).enter().append('circle')
...
function displayImmigration(immigration, origin) {
  ...
  const immigrationTip = d3.tip()
    .attr('class', 'd3-tip')
    .offset([-12, 0])
    .html(d => `Total immigration : <span style="color: hsla(0, 100%, 50%, .75);">${total}</span>`)
